...
describe('joinEndpointPath', () => {
  ...
  it('menolak path traversal termasuk encoding URL', () => {
    expect(() => joinEndpointPath('https://api.example.com/v1', '/../secret')).toThrow(/parent-directory/);
    expect(() => joinEndpointPath('https://api.example.com/v1', '/%2E%2E/secret')).toThrow(/parent-directory/);
    expect(() => joinEndpointPath('https://api.example.com/v1', '/..\\secret')).toThrow(/parent-directory/);
  });
});
...
describe('createEndpointProfile', () => {
  it('membuat profile valid dengan default Responses API', () => {
    ...
    expect(profile.compat.usagePath).toBeNull();
  });
