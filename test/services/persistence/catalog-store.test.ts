import { createEndpointProfile, joinEndpointPath } from '../../../src/domain/endpoint';
...
import {
  ...
  writeSnapshot,
  type CatalogStorage,
} from '../../../src/services/persistence/catalog-store';
...
describe('endpoint catalog cleanup', () => {
  it('removes the endpoint from both active and backup overrides', async () => {
    const { storage, files } = memoryStorage({
      'model-overrides.json': JSON.stringify({
        schemaVersion: 1,
        endpoints: {
      ...
      'model-overrides.json.backup': JSON.stringify({
        schemaVersion: 1,
        endpoints: { ep_1: { models: { old: { displayName: 'Old' } } } },
      }),
      'model-overrides.json.corrupt': JSON.stringify({
        schemaVersion: 1,
        endpoints: { ep_1: { models: { corrupt: { displayName: 'Corrupt' } } } },
      }),
    });

    await removeEndpointCatalogData(storage, 'ep_1');

    expect(files.get('model-overrides.json')).not.toContain('ep_1');
    expect(files.get('model-overrides.json.backup')).not.toContain('ep_1');
    expect(files.get('model-overrides.json.corrupt')).not.toContain('ep_1');
    expect(files.get('model-overrides.json')).toContain('ep_2');
  });
});
