...
test('conversation export redacts secrets and import creates a new ID', async () => {
  const state = setup();
  const started = await start(state.repository, 'turn_export', null, 'Use sk-secret-value');
  await state.repository.finishTurn({
    turnId: 'turn_export',
    assistantItemId: 'assistant_turn_export',
    status: 'completed',
    text: 'Authorization: Bearer token-secret',
    reasoningSummary: null,
    responseId: 'resp_export',
    usage: {
      input_tokens: 2,
      output_tokens: 3,
      provider_note: 'Authorization: Bearer usage-token-secret',
    },
    timing: { requestStart: 1, firstEvent: 2, firstVisibleToken: 3, completed: 4 },
  });

  const exported = await state.repository.exportConversation(
    started.conversationId,
    '2026-09-24T00:00:00.000Z',
  );
  assert.equal(exported.sourceConversationId, started.conversationId);
  assert.equal(JSON.stringify(exported).includes('sk-secret-value'), false);
  assert.equal(JSON.stringify(exported).includes('token-secret'), false);
  assert.equal(JSON.stringify(exported).includes('usage-token-secret'), false);

  const importedId = await state.repository.importConversation(exported);
  assert.notEqual(importedId, started.conversationId);
  const imported = await state.repository.loadConversation(importedId);
  assert.equal(imported.endpointId, 'endpoint_1');
  assert.equal(imported.modelId, 'model-exact');
  assert.equal(imported.messages.length, 2);
  assert.equal(imported.metrics[0].usage.inputTokens, 2);
});

test('removeByEndpoint leaves other endpoint conversations intact', async () => {
  const state = setup();
  const first = await start(state.repository, 'turn_ep_1', null, 'first');
  const second = await state.repository.startTurn({
    conversationId: null,
    turnId: 'turn_ep_2',
    userItemId: 'user_ep_2',
    assistantItemId: 'assistant_ep_2',
    prompt: 'second',
    attachments: [],
  ...
  assert.equal(await state.repository.countByEndpoint('endpoint_1'), 1);
  await state.repository.removeByEndpoint('endpoint_1');
  assert.equal(await state.repository.loadConversation(first.conversationId), null);
  assert.notEqual(await state.repository.loadConversation(second.conversationId), null);
});
