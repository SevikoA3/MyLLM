...
test('connect sukses menyimpan key di secure store dan profile tanpa secret', async () => {
  ...
  const profile = await createEndpointStore(stores.keyValueStore).load();
  assert.equal(stores.kv.get('myllm.activeModel.' + profile.id), 'gpt-4o-mini');
  assert.equal(profile.name, '127.0.0.1');
});
