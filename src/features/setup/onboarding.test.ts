...
import type { SecureStoreLike } from '../../services/credentials/store';
import type { KeyValueStore } from '../../services/persistence/endpoint-store';
import { connectAndDiscover, profileFromInput, suggestName, type SetupInput } from './onboarding';
...
describe('connectAndDiscover', () => {
  ...
  it('menyimpan credential dan profile saat discovery berhasil', async () => {
    ...
    const stored = JSON.parse(kv.items.get('myllm.activeEndpoint') ?? '{}');
    expect(stored.credentialRef).toBe(credentialKey.replace('myllm.credential.', ''));
    expect(kv.items.get('myllm.activeModelId')).toBe('amanai/medium');
    expect(kv.items.get('myllm.activeEndpoint')).not.toContain('sk-lokal');
  });
